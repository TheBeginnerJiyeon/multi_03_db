/**connection Pool의 이해
   ...
   - 데이터베이스 접속 설정 객체를 미리 만들어 연결을 하여 메모리(버퍼)상에 등록하여 둠으로 클라이언트가 빠르게 데이터베이스 접속을 할 수 있습니다.
    ...
    public synchronized void freeConnection(Connection c) {
        ...
        for (int i = 0; i < connections.size(); i++) {
            ...
            if ((i + 1) > _openConnections && !co.inUse)
                // 앞에 > 부터 고려하고 && 고려함
                // Within the loop, this conditional statement checks if the index (i + 1) is greater than the _openConnections threshold and if the co object's inUse property is false. If both conditions are met, it calls the removeConnection method, passing the connection associated with the co object as an argument.
