...
CEIL (abs(HIREDATE -SYSDATE)), CEIL (SYSDATE - hiredate)
FROM emp;
...
SELECT TO_CHAR(SYSDATE,'YYYY-fmMM-DD DAY') FROM DUAL; -- 월 앞에 0 제거
...
SELECT 
    ...
    DECODE (SUBSTR(EMP_NO, 8,1), '1', '남자', '여자'  ) AS GENDER   
    
FROM (
    SELECT '리사' ENAME1 , '123456-2890123' AS EMP_NO FROM DUAL
    UNION ALL --UNION ALL은 모든 결과를 단순히 결합
    SELECT '주혁' ENAME1 , '234567-1901234' AS EMP_NO FROM DUAL
    UNION ALL
    SELECT '사나' ENAME1 , '345678-2012345' AS EMP_NO FROM DUAL
);
...
SELECT JOB, COUNT(SAL), SUM(SAL) AS TOT_SAL, AVG(SAL), MIN(SAL), MAX(SAL)
FROM EMP
GROUP BY JOB
HAVING COUNT(SAL)>=4
...
DROP TABLE BBS;	-- 기존 존재하는 테이블 삭제. 편한대로 테이블 생성
	
CREATE TABLE BBS (
    NO NUMBER PRIMARY KEY,
    TITLE VARCHAR2(100),
    CONTENT VARCHAR2(500),
    WRITER NUMBER REFERENCES MEMBER(ID) -- 내 멤버 따라서 해주기
);
...
MAXVALUE 100            -- 최대100까지 -- 최대값 안적으면 됨
NOCYCLE                 -- 100이후 에러발생
NOCACHE;                -- 캐쉬사용안함
...
CREATE TABLE BBS7
AS 
SELECT * FROM BBS WHERE 1=0;


SELECT * FROM BBS7;

INSERT INTO BBS7 
VALUES (BBS_ID_SEQ.NEXTVAL, 'HAPPY', '아 ㅋㅋ' , 1);
