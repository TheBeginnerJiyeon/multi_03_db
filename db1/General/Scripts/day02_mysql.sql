...
-- 데이터 무결성 보장(데이터가 어떤경로로 들어오든 데이터에 오류가 있으면 안됨 - 제약조건을 두어 반드시 들어가야
--  하는 데이터 , 중복되면 안되는 데이터등을 관리 )


-- 스키마(schema)는 데이터베이스에서 데이터의 구조, 조직, 그리고 관계를 정의하는 틀 또는 청사진을 의미 
-- 데이터베이스 스키마는 데이터가 어떻게 저장될지, 데이터 간에 어떤 관계가 있는지, 데이터에 어떤 제약 조건이 
-- 적용되는지 등을 명시
...
-- 오라클에서 스키마를 생성한다는 것은 사실상 사용자를 생성하는 것이며 해당 사용자에게 
-- 소속된 데이터베이스 객체를 관리하는 것
-- 오라클 데이터베이스는 여러 스키마를 포함할 수 있으며, 각 스키마는 고유한 사용자에 의해 소유되고 관리됨

-- MySQL에서의 데이터베이스
-- MySQL에서 "데이터베이스"는 스키마와 거의 동일한 개념으로 사용됨 .
-- MySQL에서 데이터베이스를 생성하면, 그 데이터베이스 내에 테이블, 뷰, 
-- 인덱스 등을 생성할 수 있는 논리적 컨테이너로 취급하며, 각 데이터베이스는 서로 독립적인 객체 집합을 가짐.
-- MySQL에서는 사용자와 데이터베이스가 명시적으로 연결되지 않습니다. 대신, 
-- 사용자에게 특정 데이터베이스에 대한 접근 권한을 부여할 수 있습니다.

-- 차이점
-- 오라클에서는 "스키마"가 사용자와 직접 연결되어 사용자별로 데이터베이스 객체를 관리하는 개념. 
-- 반면, MySQL에서는 "데이터베이스"가 이러한 역할을 하며, 사용자는 별도로 권한을 부여받아 
-- 여러 데이터베이스에 접근할 수 있다. 

-- 구조적 차이: 오라클에서는 데이터베이스 내부에 여러 스키마(사용자)가 존재할 수 있으며, 
-- 각 스키마는 자신의 객체를 가짐
--          MySQL에서는 각 데이터베이스가 독립적인 객체 집합을 가지며, 
-- 사용자는 권한에 따라 하나 이상의 데이터베이스에 접근가능함
-- 관리 방식: 오라클에서는 스키마(사용자) 생성 시 데이터베이스 객체에 대한 권리가 함께 부여되고 
--          MySQL에서는 데이터베이스를 생성한 후, 별도로 사용자에게 해당 데이터베이스에 대한 
-- 접근 권한을 설정해야함
...
-- mysql 8.0.17 버전부터 int 타입에 길이 설정하는 건 더 이상 지원하지 않기로 하고 향후 버전부터는 
-- 제거될 기능 중 하나라서 경고
